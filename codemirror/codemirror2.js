// CodeMirror2 version 3.0
//
// CodeMirror2 is the only global var we claim
window.CodeMirror2 = (function() {
  "use strict";
  ...
  function CodeMirror2(place, options) {
    ...
    display.wrapper.CodeMirror2 = this;
    updateGutters(this);
    ...
    if (options.lineWrapping)
      this.display.wrapper.className += " CodeMirror2-wrap";
  ...
  function makeDisplay(place) {
    ...
    d.scrollbarH = elt("div", [elt("div", null, null, "height: 1px")], "CodeMirror2-hscrollbar");
    d.scrollbarV = elt("div", [elt("div", null, null, "width: 1px")], "CodeMirror2-vscrollbar");
    d.scrollbarFiller = elt("div", null, "CodeMirror2-scrollbar-filler");
    // DIVs containing the selection and the actual code
    d.lineDiv = elt("div");
    d.selectionDiv = elt("div", null, null, "position: relative; z-index: 1");
    // Blinky cursor, and element used to ensure cursor fits at the end of a line
    d.cursor = elt("pre", "\u00a0", "CodeMirror2-cursor");
    // Secondary cursor, shown when on a 'jump' in bi-directional text
    d.otherCursor = elt("pre", "\u00a0", "CodeMirror2-cursor CodeMirror2-secondarycursor");
    // Used to measure text size
    d.measure = elt("div", null, "CodeMirror2-measure");
    // Wraps everything that needs to exist inside the vertically-padded coordinate system
    d.lineSpace = elt("div", [d.measure, d.selectionDiv, d.lineDiv, d.cursor, d.otherCursor],
                         null, "position: relative; outline: none");
    // Moved around its parent to cover visible view
    d.mover = elt("div", [elt("div", [d.lineSpace], "CodeMirror2-lines")], null, "position: relative");
    // Set to the height of the text, causes scrolling
    d.sizer = elt("div", [d.mover], "CodeMirror2-sizer");
    // D is needed because behavior of elts with overflow: auto and padding is inconsistent across browsers
    d.heightForcer = elt("div", "\u00a0", null, "position: absolute; height: " + scrollerCutOff + "px");
    // Will contain the gutters, if any
    d.gutters = elt("div", null, "CodeMirror2-gutters");
    d.lineGutter = null;
    // Helper element to properly size the gutter backgrounds
    var scrollerInner = elt("div", [d.sizer, d.heightForcer, d.gutters], null, "position: relative; min-height: 100%");
    // Provides scrolling
    d.scroller = elt("div", [scrollerInner], "CodeMirror2-scroll");
    d.scroller.setAttribute("tabIndex", "-1");
    // The element in which the editor lives.
    d.wrapper = elt("div", [d.inputDiv, d.scrollbarH, d.scrollbarV,
                            d.scrollbarFiller, d.scroller], "CodeMirror2");
  ...
  function loadMode(cm) {
    var doc = cm.view.doc;
    cm.view.mode = CodeMirror2.getMode(cm.options, cm.options.mode);
  ...
  function wrappingChanged(cm) {
    ...
    if (cm.options.lineWrapping) {
      cm.display.wrapper.className += " CodeMirror2-wrap";
      ...
      cm.display.wrapper.className = cm.display.wrapper.className.replace(" CodeMirror2-wrap", "");
  ...
  function updateGutters(cm) {
    ...
    for (var i = 0; i < specs.length; ++i) {
      ...
      var gElt = gutters.appendChild(elt("div", null, "CodeMirror2-gutter " + gutterClass));
      if (gutterClass == "CodeMirror2-linenumbers") {
  ...
  // "CodeMirror2-linenumbers" when the lineNumbers option is true.
  function setGuttersForLineNumbers(options) {
    var found = false;
    for (var i = 0; i < options.gutters.length; ++i) {
      if (options.gutters[i] == "CodeMirror2-linenumbers") {
        if (options.lineNumbers) found = true;
        else options.gutters.splice(i--, 1);
      }
    }
    if (!found && options.lineNumbers)
      options.gutters.push("CodeMirror2-linenumbers");
  }
  ...
  function maybeUpdateLineNumberWidth(cm) {
    ...
    if (last.length != display.lineNumChars) {
      var test = display.measure.appendChild(elt("div", [elt("div", last)],
                                                 "CodeMirror2-linenumber CodeMirror2-gutter-elt"));
  ...
  function buildLineElement(cm, line, lineNo, dims) {
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (cm.options.lineNumbers && (!markers || !markers["CodeMirror2-linenumbers"]))
        wrap.lineNumber = gutterWrap.appendChild(
          elt("div", lineNumberFor(cm.options, lineNo),
              "CodeMirror2-linenumber CodeMirror2-gutter-elt",
              "left: " + dims.gutterLeft["CodeMirror2-linenumbers"] + "px; width: "
              + display.lineNumInnerWidth + "px"));
      if (markers)
        for (var k = 0; k < cm.options.gutters.length; ++k) {
          var id = cm.options.gutters[k], found = markers.hasOwnProperty(id) && markers[id];
          if (found)
            gutterWrap.appendChild(elt("div", [found], "CodeMirror2-gutter-elt", "left: " +
                                       dims.gutterLeft[id] + "px; width: " + dims.gutterWidth[id] + "px"));
        }
    }
    // Kludge to make sure the styled element lies behind the selection (by z-index)
    if (line.bgClass)
      wrap.appendChild(elt("div", "\u00a0", line.bgClass + " CodeMirror2-linebackground"));
    wrap.appendChild(lineElement);
    if (line.widgets)
      for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
        var widget = ws[i], node = elt("div", [widget.node], "CodeMirror2-linewidget");
        node.widget = widget;
        if (widget.noHScroll) {
  ...
  function updateSelectionRange(cm) {
    ...
    function add(left, top, width, bottom) {
      if (top < 0) top = 0;
      fragment.appendChild(elt("div", null, "CodeMirror2-selected", "position: absolute; left: " + left +
  ...
  function mouseEventInWidget(display, e) {
    for (var n = e_target(e); n != display.wrapper; n = n.parentNode)
      if (/\bCodeMirror2-(?:line)?widget\b/.test(n.className) ||
  ...
  function onFocus(cm) {
    ...
    if (!cm.view.focused) {
      ...
      if (cm.display.scroller.className.search(/\bCodeMirror2-focused\b/) == -1)
        cm.display.scroller.className += " CodeMirror2-focused";
      resetInput(cm, true);
    }
    slowPoll(cm);
    restartBlink(cm);
  ...
  function onBlur(cm) {
    if (cm.view.focused) {
      ...
      cm.display.scroller.className = cm.display.scroller.className.replace(" CodeMirror2-focused", "");
    }
  ...
  CodeMirror2.prototype = {
    ...
    toggleOverwrite: function() {
      if (this.view.overwrite = !this.view.overwrite)
        this.display.cursor.className += " CodeMirror2-overwrite";
      else
        this.display.cursor.className = this.display.cursor.className.replace(" CodeMirror2-overwrite", "");
    },
  ...
  var optionHandlers = CodeMirror2.optionHandlers = {};

  // The default configuration options.
  var defaults = CodeMirror2.defaults = {};
  ...
  var Init = CodeMirror2.Init = {toString: function(){return "CodeMirror2.Init";}};
  ...
  var modes = CodeMirror2.modes = {}, mimeModes = CodeMirror2.mimeModes = {};
  ...
  CodeMirror2.resolveMode = function(spec) {
    if (typeof spec == "string" && mimeModes.hasOwnProperty(spec))
      spec = mimeModes[spec];
    else if (typeof spec == "string" && /^[\w\-]+\/[\w\-]+\+xml$/.test(spec))
      return CodeMirror2.resolveMode("application/xml");
    if (typeof spec == "string") return {name: spec};
    else return spec || {name: "null"};
  };

  CodeMirror2.getMode = function(options, spec) {
    var spec = CodeMirror2.resolveMode(spec);
    var mfactory = modes[spec.name];
    if (!mfactory) return CodeMirror2.getMode(options, "text/plain");
  ...
  var modeExtensions = CodeMirror2.modeExtensions = {};
  CodeMirror2.extendMode = function(mode, properties) {
  ...
  var initHooks = [];
  CodeMirror2.defineInitHook = function(f) {initHooks.push(f);};
  ...
  CodeMirror2.innerMode = function(mode, state) {
  ...
  function markText(cm, from, to, options, type) {
    ...
    if (marker.replacedWith) {
      ...
      marker.replacedWith = elt("span", [marker.replacedWith], "CodeMirror2-widget");
    }
  ...
  CodeMirror2.on = on; CodeMirror2.off = off; CodeMirror2.signal = signal;
  ...
  // handling this'.
  var Pass = CodeMirror2.Pass = {toString: function(){return "CodeMirror2.Pass";}};
